/**
 * ✨ Voice Changer HololiveID (RVC) via POST /ai/openai
 * Creator: RyuuDev
 */

const express = require('express'); // Jika kamu belum import express di tempat lain
const axios = require('axios');
const FormData = require('form-data');
const textToSpeech = require('google-tts-api'); // npm install google-tts-api
const fs = require('fs');
...
module.exports = function (app) {
    app.post('/ai/rvc', async (req, res) => {
        const { text, model = 'risu', transpose = 0 } = req.body;
        ...
        try {
            // Ubah text jadi suara (TTS)
            const ttsUrl = textToSpeech.getAudioUrl(text, {
                lang: 'en',
                slow: false,
                host: 'https://translate.google.com',
            });

            const tmpFilePath = path.resolve(__dirname, `tts_${Date.now()}.mp3`);
            ...
            const audioResult = await rvc.process(buffer, { model, transpose });

            fs.unlinkSync(tmpFilePath); // hapus file temp

            res.status(200).json({
                status: true,
                message: 'Berhasil diubah ke suara karakter HololiveID',
